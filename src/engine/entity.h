...
public:
	...
	public:
		...
		{
			return _actual_component_count;
	...
	inline void init()
	{
		...
		_is_init = true;
		printf("ACTUAL COMPONENT COUNT : %ld\n", global_component_index);

		assert_error(global_component_index <= TS_MAXIMUM_COMPONENT, "");

		_actual_component_count = global_component_index;
		_components = new std::shared_ptr<ComponentBase>[maximum_entity * _actual_component_count];
		_component_vectors.reserve(maximum_entity);
		_entity_vector.reserve(maximum_entity);

		std::size_t index = _actual_component_count;
		for (std::size_t i = 1; i < maximum_entity; i++, index += _actual_component_count) {
...
private:
	...
	static std::size_t _actual_component_count;
	bool _is_init = false;
};
