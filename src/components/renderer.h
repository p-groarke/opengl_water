...
struct Renderer {
	...
	void create();
	void init(ts::Entity entity); // override;
	void update(ts::Entity entity, float dt); // override;
	void render(ts::Entity entity, float dt); // override;
	void destroy(ts::Entity entity); // override;
...
private:
	...
	Shader _shaders[_shader_count] = { { GL_VERTEX_SHADER }, { GL_TESS_CONTROL_SHADER },
